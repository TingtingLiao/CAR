...
class MetaMLP(MetaModule):
    def __init__(self,
        ...
        self.activation = nn.Softplus(beta) if beta > 0 else nn.ReLU(True)
        self.skip_in = skip_in

        self.num_layers = len(dims)
        self.net = []
        for layer in range(0, self.num_layers - 1):
            out_dim = dims[layer + 1] - dims[0] if layer + 1 in skip_in else dims[layer + 1]

            # lin = nn.Linear(dims[layer], out_dim)
            lin = BatchLinear(dims[layer], out_dim)
    ...
    def forward(self, points, params=None, return_feats=False):
        x = points
        ...
        for i, layer in enumerate(self.net):
            ...
            if params is not None:
                x = layer(x, params=self.get_subdict(self.get_subdict(params, 'net'), str(i)))
            else:
                x = layer(x)
...
def gradient(inputs, outputs):
    d_points = torch.ones_like(outputs, device=outputs.device)
...
def last_hyper_init(m):
    if type(m) == nn.Linear or type(m) == nn.Conv1d or type(m) == BatchLinear:
