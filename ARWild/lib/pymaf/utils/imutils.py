...
def process_image(img_file, det, mask_file, input_res=512):
    ...
    mask_ori = cv2.imread(mask_file)
    mask_for_crop = cv2.warpAffine(mask_ori, M[0:2, :], (input_res * 2, input_res * 2), flags=cv2.INTER_CUBIC)
    mask_np = crop(mask_for_crop, center, scale, (input_res, input_res))
    img_mask = torch.from_numpy(mask_np / 255.).max(-1)[0].unsqueeze(0).int()

    img_tensor = torch.from_numpy(img_np).permute(2, 0, 1) / 255. * 2 - 1
    img_tensor = img_tensor.float() * img_mask.expand_as(img_tensor)

    img_hps = Image.fromarray(np.uint8(img_np))
    img_hps = image_to_pymaf_tensor(img_hps).unsqueeze(0).float()
