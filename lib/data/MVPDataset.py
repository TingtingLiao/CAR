...
class MVPDataset(Dataset):
    ...
    def __init__(self, cfg, phase='train'):
        ...
        self.sample_dir = self.opt.mvp_sample_dir
        self.smpl_fit_dir = self.opt.mvp_smpl_dir
        ...
        # self.subjects = ['234359'] if self.overfit else self.get_subjects()
        self.subjects = self.get_subjects()
        if self.overfit:
            self.subjects = self.subjects[:5]
        # self.subjects = ["197109"]
    ...
    def get_subjects(self):
        if self.is_train:
            subjects = sorted(Path('./data/splits/mvp_train200.txt').read_text().strip().split('\n'))
        else:
            subjects = sorted(Path('./data/splits/mvp_test50.txt').read_text().strip().split('\n'))
        return subjects
    ...
    def get_render(self, sid):
        ...
        for aid in aids:
            ...
            render_path = '%s/%s/%s/RENDER/000.png' % (self.syn_dir, sid, aid)
        ...
        return {
            'rgb': torch.stack(render_list).float(),
    ...
    def sampling_sdf_cache(self, subject, joint_transform, calib):
        ...
        posed_pts, projected_pts, vT = warp_and_project_points(canon_points, weights, joint_transform, calib, True)
        return {
            'canon_points': canon_points.t().float(),  # [3, N]
            'posed_points': posed_pts.transpose(1, 2).float(),  # [num_view, 3, num_surf]
            'projected_points': projected_pts.transpose(1, 2).float(),  # [num_view, 3, num_surf]
    ...
    def sampling_sdf_from_mesh(self, subject, joint_transform, calib):
        obj_file = os.path.join(self.opt.mvp_obj_dir, subject, 'da-pose.obj')
        skin_file = os.path.join(self.opt.mvp_obj_dir, subject, 'skin_weight.npz')
