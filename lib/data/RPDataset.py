...
import smpl
from lib.common.lbs_util import warp_and_project_points
from lib.common.train_util import query_lbs_weight
...
class RenderPeople(Dataset):
    ...
    def __init__(self, cfg, phase='train'):
        ...
        self.sample_dir = self.opt.rp_sample_dir
        self.smpl_dir = self.opt.rp_smpl_dir
        ...
        self.n_rotation = 120 if self.is_train else 1
        self.n_action = 10 if self.is_train else 1
    ...
    def get_render(self, sid):
        ...
        if self.is_train:
            ...
            aids = [f'{i:03d}' for i in range(self.num_views)]

        for aid in aids:
            if self.is_train:
                rgb_path = random.choice(glob('%s/%s/%s/RENDER/*.png' % (self.syn_dir, sid, aid)))
            else:
                rgb_path = '%s/%s/%s/RENDER/000.png' % (self.syn_dir, sid, aid)

            rid = rgb_path.split('/')[-1][:-4]

            mask_path = '%s/%s/%s/MASK/%s.png' % (self.syn_dir, sid, aid, rid)
            param_path = '%s/%s/%s/PARAM/%s.npz' % (self.syn_dir, sid, aid, rid)
            normal_path = '%s/%s/%s/NORMAL/%s.png' % (self.syn_dir, sid, aid, rid)
            ...
            pose = torch.from_numpy(param['pose']).view(24, 3)
            ...
            normal = mask.expand_as(render) * self.to_tensor(normal)

            rgb_list.append(render)
        ...
        return {
            'rgb': torch.stack(rgb_list).float(),
            'normal': torch.stack(normal_list).float(),
    ...
    @torch.no_grad()
    def load_smpl(self, subject, poses, calib):
        """
        Args:
            subject: str
        ...
        # canonical smpl data
        smpl_data = np.load(os.path.join(self.smpl_dir, subject, 'param.npz'))
        betas = torch.from_numpy(smpl_data['beta']).view(1, -1).contiguous()

        if self.cfg.smpl.model_type == 'smpl_vitruvian':
            canon_vertices, canon_joints = self.smpl_model.initiate_vitruvian(
                betas=betas, vitruvian_angle=self.cfg.smpl.vitruvian_angle, custom_out=True)
        else:
        ...
        for pose in poses:
            posed_out = self.smpl_model(
                betas=betas, global_orient=pose[None, :1], body_pose=pose[None, 1:], custom_out=True)
            posed_smpl_verts.append(posed_out.vertices)
            posed_joints.append(posed_out.joints[:, :24])

            # project_smpl_verts.append(orthogonal(posed_out.vertices.transpose(1, 2), calib).transpose(1, 2))

        return {
            'canon_smpl_vert': canon_vertices[0].float(),  # [N, 3]
            'posed_smpl_vert': torch.cat(posed_smpl_verts).float(),  # [num_view, N, 3]
            # 'project_smpl_verts': torch.cat(project_smpl_verts).float(),  # [num_view, N, 3]
    ...
    def sampling_sdf_cache(self, subject, joint_transform, calib):
        ...
        # save_obj_mesh('mesh.obj', sample_data['bbox_points'].numpy())
        # save_obj_mesh('mesh.obj', canon_points.numpy())
        # print('saved')
        # exit()
        ...
        posed_points, projected_points, vT = warp_and_project_points(canon_points, weights, joint_transform, calib,
                                                                     True)
    ...
    def sampling_sdf_from_mesh2(self, subject, joint_transform, calib):
        obj_file = os.path.join(self.smpl_dir, subject, 'da_pose.obj')
        skin_file = os.path.join(self.smpl_dir, subject, 'skin_weight.pt')

        canon_mesh = trimesh.load(obj_file, **{'process': False, 'maintain_order': True})
        skin_weights = torch.load(skin_file)
    ...
    def sampling_sdf_from_mesh(self, subject, joint_transform, calib):
        ...
        canon_mesh = trimesh.load(obj_file, **{'process': False, 'maintain_order': True})
        skin_weights = torch.load(skin_file)
        posed_mesh_v = warp_and_project_points(canon_mesh.vertices, skin_weights, joint_transform)[0].numpy()
        posed_mesh = trimesh.Trimesh(posed_mesh_v, canon_mesh.faces, **{'process': False, 'maintain_order': True})

        # sampling on surface
        N = canon_mesh.vertices.shape[0]
        ids = np.random.choice(np.arange(N), self.opt.num_surface, p=np.ones(N).astype(np.float32) / N)
        # ids = np.arange(N)
        p_surf_pts = posed_mesh.vertices[ids]
        p_surf_nml = compute_normal(posed_mesh.vertices, posed_mesh.faces)[ids]
        c_surf_nml = compute_normal(canon_mesh.vertices, canon_mesh.faces)[ids]
        p_surf_nml = torch.as_tensor(p_surf_nml).t().float()
        c_surf_nml = torch.as_tensor(c_surf_nml).t().float()

        # sampling around surface
        perturb_pts = p_surf_pts + np.random.normal(scale=self.opt.sigma, size=p_surf_pts.shape)
        # sampling uniform
        calib_inv = torch.inverse(calib[0]).numpy()
        pr_bbox_pts = np.random.rand(self.opt.num_bbox, 3) * 2 - 1
        p_bbox_pts = np.matmul(calib_inv[:3, :3], pr_bbox_pts.T).T + calib_inv[:3, 3]

        off_surf_pts = np.concatenate([perturb_pts, p_bbox_pts])
        off_weights = query_lbs_weight(off_surf_pts, posed_mesh.vertices, skin_weights)
        posed_points = np.concatenate([p_surf_pts, off_surf_pts])
        weights = np.concatenate([skin_weights[ids], off_weights])

        canon_points, projected_points, vT = warp_and_project_points(
            posed_points, weights, joint_transform, calib, return_vT=True, inverse=True)

        rot = calib[:, :3, :3]
        pr_surf_nml = torch.bmm(rot, p_surf_nml[None])  # [B, 3, N]
        pr_surf_nml = F.normalize(pr_surf_nml)

        return {
            'canon_points': canon_points[0].t().float(),  # [3, N]
            'posed_points': torch.as_tensor(posed_points)[None].transpose(1, 2).float(),  # [num_view, 3, num_surf]
            'projected_points': projected_points.transpose(1, 2).float(),  # [num_view, 3, num_surf]
            'canon_surf_normal': c_surf_nml.float(),  # [3, num_surf]
            'posed_surf_normal': p_surf_nml[None].float(),  # [B, 3, num_surf]
            'projected_surf_normal': pr_surf_nml.float(),  # [3, num_surf]
            'vT': vT[0],
        }
    ...
    def get_item(self, index):
        ...
        data_dict.update(self.load_smpl(sid, data_dict['poses'], data_dict['calib']))

        if self.sdf:
            data_dict.update(self.sampling_sdf_from_mesh(sid, data_dict['joint_transform'], data_dict['calib']))
