...
class NVDiffRender(torch.nn.Module):
    def __init__(self):
    ...
    def forward(self, vertices, faces, h=512, w=512, spp=1):
        ...
        rast, rast_db = dr.rasterize(self.glctx, vertices, faces, res)
        ...
        alpha, _ = dr.interpolate(torch.ones_like(vertices[..., :1]), rast, faces)  # [B, H, W, 1]

        # compute normal
        from lib.data.mesh_util import compute_normal_torch
        vn = compute_normal_torch(vertices[0], faces)
        normal, _ = dr.interpolate(vn[None, ...].float(), rast, faces)

        ################################################################################
        # Shade
        ################################################################################
        normal = (normal + 1) / 2.
        normal = dr.antialias(normal, rast, vertices, faces).clamp(0, 1)  # [H, W, 3]
        alpha = dr.antialias(alpha, rast, vertices, faces).clamp(0, 1)  # [H, W, 3]
...
if __name__ == '__main__':
    mesh = trimesh.load("./out/res/car-normal-1view/rp/rp_lilly_rigged_004_MAX.obj")
    render = NVDiffRender()
    v = torch.as_tensor(mesh.vertices).cuda()
    f = torch.as_tensor(mesh.faces).long().cuda()
    render(v, f)
