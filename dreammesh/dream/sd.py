...
if __name__ == '__main__':
    ...
    # "clinton": "Hilary Clinton, full body",
    #
    ...
    imgs = [
        np.hstack([
            sd.prompt_to_img(f"{prompt}; head; {v} view;", opt.negative, opt.H, opt.W, opt.steps)[0]
            for v in ["front", "back", "side"]
        ])
        for prompt in ["superman", "Donald Trump", "Robot", "antman"]
    ]

    cv2.imwrite("superman.png", np.vstack(imgs)[..., ::-1])
    # visualize image
    # plt.imshow(imgs[0])
    # plt.show()
