...
class Mesh():
    def __init__(self, v=None, f=None, vn=None, fn=None, vt=None, ft=None, albedo=None, device=None, base=None,
        ...
        if init_empty_tex and albedo is not None:
            self.albedo = np.ones((1024, 1024, 3), dtype=np.float32) * np.array([0.5, 0.5, 0.5])  # default color
        else:
            self.albedo = albedo
        self.device = device
    ...
    def auto_normal(self):
        self.vn = compute_normal(self.v, self.f)
        self.fn = self.f
