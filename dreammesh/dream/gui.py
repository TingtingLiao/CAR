...
class GUI:
    def __init__(self, opt, trainer, debug=True):
        ...
        self.W = opt.W
        self.H = opt.H
        self.cam = OrbitCamera(opt.W, opt.H, r=opt.radius, fovy=opt.fovy, near=opt.min_near)
        self.debug = debug
    ...
    def test_step(self):
        ...
        if self.need_update or self.spp < self.opt.max_spp:
            ...
            outputs = self.trainer.test_gui(self.cam.pose, self.cam.intrinsics, mvp, self.W, self.H, self.bg_color,
                                            self.spp, self.downscale, self.light_dir, self.ambient_ratio, self.shading)
            ...
            with dpg.collapsing_header(label="Options", default_open=True):
                ...
                with dpg.group(horizontal=True):
                    dpg.add_slider_float(label="theta", min_value=0, max_value=180, format="%.2f",
                                         default_value=self.opt.light_theta, callback=callback_set_light_dir,
                                         user_data=0)

                with dpg.group(horizontal=True):
                    dpg.add_slider_float(label="phi", min_value=0, max_value=360, format="%.2f",
                                         default_value=self.opt.light_phi, callback=callback_set_light_dir, user_data=1)
                ...
                dpg.add_combo(('albedo', 'lambertian', 'textureless', 'normal'), label='shading',
                              default_value=self.shading, callback=callback_change_shading)
